...
# FIX BUG 1: Penambahan fungsi normalize_url yang hilang
...
def analyze_with_gemini(article):
    ...
    if not client:
        return {
            ...
            "ringkasan": f"Laporan berita ini membahas mengenai {title_text.lower()} di Kabupaten Lamongan."
        }

    prompt = f"""
Anda adalah analis berita ekonomi Badan Pusat Statistik (BPS) Kabupaten Lamongan.

TENTUKAN APAKAH BERITA INI BERITA EKONOMI KABUPATEN LAMONGAN ATAU BUKAN.
- Jika ekonomi=true, tentukan kode sektor BPS (A, B, C, D, E, F, G, H, I, J, K, L, MN, O, P, Q, RSTU) dan ringkasan deskriptif (maks 25 kata).
- DILARANG KERAS membuat ringkasan yang sama persis dengan judul!

Judul: {title_text}
Isi: {content_text}

Jawab HANYA JSON format:
{{
    "ekonomi": true,
    "sektor_kode": "G",
    "isu_ekonomi": "UMKM",
    "ringkasan": "Penjelasan deskriptif berita."
}}
"""
    ...
    try:
        ...
        sektor_name = SEKTOR_BPS.get(code, match_fallback_sector(title_text))
...
# ============================================================
# 📥 PENGAMBILAN BERITA RSS
# ============================================================

def fetch_and_process_news():
    topics = ["Lamongan", "Kabupaten Lamongan", "Pemkab Lamongan"]
    raw_articles = []
    ...
    status.success(f"✅ Selesai! Berhasil menganalisis {len(final_records)} berita ekonomi Lamongan.")
    progress.empty()
